    ...
    else
    {
      ...
      if (type == 0)
      {
        try
        {
          ...
          buddy.println(start+" "+end+" Search type 0");
        }
        catch (Exception e)
        {
        ...
        try
        {
          ...
          stmt = conn
              .prepareStatement("SELECT comments.id, comments.story_id, "
                  ...
                  + " OFFSET " + page * nbOfStories,
              // Yasu: FORWARD_ONLY ResultSet doesn't support first() method.
	      ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
          rs = stmt.executeQuery();
          long end=System.currentTimeMillis();
          buddy.println(start+" "+end+" Search type 1");
          if (!rs.first())
          {
            stmt.close();
            ...
            buddy.println(start1+" "+end1+" Search type 1");
      ...
      { // Look for stories of an author
        try
        {
          ...
          rs = stmt.executeQuery();
          long end1=System.currentTimeMillis();
        buddy.println(start1+" "+end1+" Search type 2");
          if (!rs.first())
          {
            stmt.close();
            ...
            buddy.println(start2+" "+end2+" Search type 2");
          }
          if (!rs.first())
          {
            if (page == 0)
