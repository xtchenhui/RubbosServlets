...
public class BrowseStoriesByCategory extends RubbosHttpServlet
{
      ...
      throws IOException, ServletException
  {
    ...
    conn = getRConnection("BrowseStoriesByCategories");
    ...
    try
    {
      ...
      stmt = conn.prepareStatement("SELECT * FROM stories WHERE category= "
          ...
          // + "," + nbOfStories,
          + categoryId + " ORDER BY date DESC LIMIT " + nbOfStories + " OFFSET "
          + page * nbOfStories,
            // Yasu: FORWARD_ONLY ResultSet doesn't support first() method.
	    ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
      rs = stmt.executeQuery();
      long end=System.currentTimeMillis();
//      buddy.println(beforestart+" "+start+" "+end+" BrowseStoriesByCategory"+" getconnectiontime:"+(start-beforestart));
      buddy.println(start+" "+end+" BrowseStoriesByCategory");
    }
    catch (Exception e)
    {
