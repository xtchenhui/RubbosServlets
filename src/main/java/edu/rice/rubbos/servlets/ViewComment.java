...
public class ViewComment extends RubbosHttpServlet
{
      ...
      throws IOException, ServletException
  {
    ...
    conn = getRConnection("ViewComment");
    ...
    else
    {
      try
      {
        ...
        buddy.println(start+" "+end+" ViewComment");
        if (!rs.first())
        {
          sp
    ...
    try
    {
      Comment.fetchAndDisplay(sp, conn, comment_table, filter, commentId, display, storyId, parent,buddy);
    }
    catch (Exception e)
    {
      sp.printHTML("Failed to execute Query for ViewComment: " + e);
