...
public class OlderStories extends RubbosHttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if ((day == null) || (month == null) || (year == null))
			...
			conn = getRConnection("OlderStories");

			try {
				long start=System.currentTimeMillis();
				stmt = conn.prepareStatement("SELECT stories.id, stories.title, " + "stories.date, users.nickname "
						+ "FROM stories, users WHERE date>='" + before + "' AND date<='" + after
						+ "' AND users.id = stories.writer"
						// Yasu: for PostgreSQL
						// + " ORDER BY date DESC LIMIT " + page * nbOfStories +
						// ","
						// + nbOfStories,
						+ " ORDER BY date DESC LIMIT " + nbOfStories + " OFFSET " + page * nbOfStories,
						// Yasu: FORWARD_ONLY ResultSet doesn't support first()
						// method.
						ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
				rs = stmt.executeQuery();
				long end=System.currentTimeMillis();
				buddy.println(start+" "+end+" "+" OlderStories");
				if (!rs.first()) {
					stmt.close();
					long start1=System.currentTimeMillis();
					stmt = conn.prepareStatement("SELECT old_stories.id, old_stories.title,"
							+ " old_stories.date, users.nickname " + "FROM old_stories, users WHERE date>='" + before
							+ "' AND date<='" + after + "' AND users.id = old_stories.writer"
							// Yasu: for PostgreSQL
							// + " ORDER BY date DESC LIMIT " + page *
							// nbOfStories + ","
							// + nbOfStories,
							+ " ORDER BY date DESC LIMIT " + nbOfStories + " OFFSET " + page * nbOfStories,
							// Yasu: FORWARD_ONLY ResultSet doesn't support
							// first() method.
							ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
					rs = stmt.executeQuery();
					long end1=System.currentTimeMillis();
					buddy.println(start1+" "+end1+" "+" OlderStories");
				}
				if (!rs.first()) {
					if (page == 0)
						...
						sp.printHTML("<p><CENTER>\n<a href=\"/rubbos/servlet/edu.rice.rubbos.servlets.OlderStories?day="
								+ day + "&month=" + month + "&year=" + year + "&page=" + (page - 1) + "&nbOfStories="
			...
			try {
				while (rs.next()) {
					...
					sp.printHTML("<a href=\"/rubbos/servlet/edu.rice.rubbos.servlets.ViewStory?storyId=" + id + "\">"
							+ title + "</a> by " + username + " on " + date + "<br>\n");
				}
			} catch (Exception e2) {
			...
			if (page == 0)
				sp.printHTML("<p><CENTER>\n<a href=\"/rubbos/servlet/edu.rice.rubbos.servlets.OlderStories?day=" + day
						+ "&month=" + month + "&year=" + year + "&page=" + (page + 1) + "&nbOfStories=" + nbOfStories
						+ "\">Next page</a>\n</CENTER>\n");
			else
				sp.printHTML("<p><CENTER>\n<a href=\"/rubbos/servlet/edu.rice.rubbos.servlets.OlderStories?day=" + day
						+ "&month=" + month + "&year=" + year + "&page=" + (page - 1) + "&nbOfStories=" + nbOfStories
						+ "\">Previous page</a>\n&nbsp&nbsp&nbsp"
						+ "<a href=\"/rubbos/servlet/edu.rice.rubbos.servlets.OlderStories?category=" + day + "=" + day
						+ "&month=" + month + "&year=" + year + "&page=" + (page + 1) + "&nbOfStories=" + nbOfStories
						+ "\">Next page</a>\n\n</CENTER>\n");
		}
		sp.printHTMLfooter();
