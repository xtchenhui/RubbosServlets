...

      ...
      throws IOException, ServletException
  {
    ...
    try
    {
      ...
      buddy.println(start+" "+end+" SubmitStory");
    }
    catch (Exception e)
    {
    ...
    try
    {
      Comment.displayFilterChooser(conn, sp, commentId, storyIdtest,
          comment_table, display, filter,buddy);
    }
    catch (Exception e)
    {
    ...
    try
    {
      Comment.fetchAndDisplay(sp, conn, comment_table, filter, commentId, display, storyIdtest, 0,buddy);
    }
    catch (Exception e)
    {
      sp.printHTML("Failed to execute Query for ViewStory: " + e);
